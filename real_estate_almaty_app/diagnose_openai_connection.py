...
Проверяет, почему программа пишет:
"ответ сгенерирован локальным ассистентом, потому что OpenAI API недоступен или ключ не настроен"

Запускать из папки проекта:
python diagnose_openai_connection.py

Скрипт НЕ печатает полный API key.
"""
...
try:
    ...
    Path("logs/openai_diagnostic_error.txt").write_text(
        traceback.format_exc(),
        encoding="utf-8"
    )
    print("\nFull error saved to: logs/openai_diagnostic_error.txt")
    ...
    if "401" in msg or "api key" in msg or "unauthorized" in msg:
        ...
        print("\nLikely reason: model not available for your project. Try:")
        print('$env:OPENAI_MODEL="gpt-5.4-mini"')
        print("or")
        print('$env:OPENAI_MODEL="gpt-4.1"')
    elif "quota" in msg or "billing" in msg or "insufficient" in msg:
        print("\nLikely reason: no API billing/credits.")
    else:
        print("\nUnknown reason. Send me the Error type and Error message above, but DO NOT send your API key.")
