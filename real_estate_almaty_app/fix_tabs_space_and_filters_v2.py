...
backup = Path("main.py.backup_tabs_space_fix_v2")
...
def ensure_qsizepolicy_import(s):
    ...
    for c in candidates:
        if c in s:
            print("OK: QSizePolicy import added")
            return s.replace(c, c + " QSizePolicy,", 1)
    print("WARNING: QSizePolicy import not added automatically")
    return s
...
def replace_method_at_existing_position(s, name, new_method, fallback_marker=None):
    ...
    if bounds:
        ...
        for start, end in reversed(bounds):
            s = s[:start] + s[end:]
        return s[:insert_at] + new_method.rstrip() + "\n" + s[insert_at:]

    if fallback_marker:
        marker = "\n    def " + fallback_marker
        if marker in s:
            print(f"Inserted {name} before {fallback_marker}")
            return s.replace(marker, "\n" + new_method.rstrip() + "\n" + marker, 1)

    print(f"WARNING: {name} not found and no fallback used")
    return s

def add_css(s):
    if "/* tabs-space-filter-compact-fix-v2 */" in s:
        return s
...
text = replace_method_at_existing_position(text, "_build_filters_panel", FILTERS_METHOD, "_init_filter_ranges")
text = replace_method_at_existing_position(text, "_build_stats_tab", STATS_METHOD, "_build_ai_tab")
text = replace_method_at_existing_position(text, "_build_ai_tab", AI_METHOD, None)

text = add_css(text)

MAIN_PATH.write_text(text, encoding="utf-8")

print("DONE: fixed filters height, removed extra hint, improved Stats and AI tab space.")
